...
import { NOTE_COLOR_OPTIONS } from '../constants/colors';
...
type ImageBlock = { type: 'image'; id: string; uri: string; widthPct: 50 | 75 | 100; height: number };
...
function loadBlocks(note: Note | undefined, draftBlocks: NoteBlock[] | null): NoteBlock[] {
  ...
  if (note.content?.startsWith('[')) {
    try { return JSON.parse(note.content) as NoteBlock[]; } catch {// ignore
      }
  }
...
const CATEGORIES: { label: string; icon: React.ComponentProps<typeof Ionicons>['name']; color: string; iconBg: string; bg: string }[] = [
...
function CategorySheet({ visible, selected, onSelect, onClose }: {
        ...
        <View style={cs.chipGrid}>
          {CATEGORIES.map(({ label, icon, color, bg }) => {
            ...
            return (
              <TouchableOpacity
                key={label}
                style={[cs.chip, active && { backgroundColor: bg, borderColor: color, borderWidth: 2 }]}
                onPress={() => { onSelect(label); onClose(); }}
                activeOpacity={0.7}
              >
                <Ionicons name={icon} size={16} color={color} />
                <Text style={[cs.chipLabel, { color }, active && { fontWeight: '700' }]}>{label}</Text>
              </TouchableOpacity>
            );
          })}
        </View>
...
function ResizeSheet({ block, onUpdate, onClose }: {
  block: ImageBlock;
  ...
  return (
    <Modal visible transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={[rs.sheet, { backgroundColor: c.surface }]}>
        ...
        <TouchableOpacity style={rs.applyBtn} onPress={() => { onUpdate(widthPct, height); onClose(); }}>
...
const rs = StyleSheet.create({
  ...
  heightRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 20, marginBottom: 28 },
  ...
  applyBtn: {
    backgroundColor: '#0EA5E9', borderRadius: 16,
...
function ImageBlockView({ block, onRemove, onResize }: {
  block: ImageBlock;
  onRemove: () => void;
  onResize: () => void;
}) {
  ...
  return (
    <View style={[ib.wrap, { width: widthMap[block.widthPct] as `${number}%` }]}>
      <Image source={{ uri: block.uri }} style={{ width: '100%', height: block.height, borderRadius: 12 }} resizeMode="cover" />
      <TouchableOpacity style={ib.removeBtn} onPress={onRemove} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
        <Ionicons name="close-circle" size={24} color="#fff" />
      </TouchableOpacity>
      <TouchableOpacity style={ib.resizeBtn} onPress={onResize} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
...
const ib = StyleSheet.create({
  ...
  removeBtn: { position: 'absolute', top: 6, right: 6, backgroundColor: 'rgba(0,0,0,0.45)', borderRadius: 14, padding: 3 },
  resizeBtn: {
    position: 'absolute', top: 6, left: 6,
    backgroundColor: 'rgba(0,0,0,0.45)', borderRadius: 14, padding: 3,
  },
});
...
const EXPENSE_CATEGORIES: ExpenseCategory[] = ['Food', 'Transport', 'Shopping', 'Health', 'Bills', 'Other'];

function ExpenseConfirmSheet({ item, index, total, onAction }: {
  item: ExpenseQueueItem; index: number; total: number;
  onAction: (confirm: boolean, title: string, amount: number, category: ExpenseCategory) => void;
}) {
  const c = useThemeColors();
  ...
  const [expCategory, setExpCategory]   = useState<ExpenseCategory>(d?.category ?? (e?.category as ExpenseCategory) ?? 'Other');
  ...
  const handleConfirm = () => {
    if (isAdd) {
      ...
      if (!expTitle.trim() || isNaN(amt) || amt <= 0) {
        showAlert({ type: 'error', title: 'Invalid Details', message: 'Please enter a valid title and amount for this expense.' });
        return;
      }
      const finalCategory = (expCategory === 'Other' && customCategory.trim())
        ? customCategory.trim() as ExpenseCategory
        : expCategory;
      onAction(true, expTitle.trim(), amt, finalCategory);
    } else {
      onAction(true, e?.title ?? '', e?.amount ?? 0, (e?.category as ExpenseCategory) ?? 'Other');
    }
  };

  const handleSkip = () => onAction(false, '', 0, 'Other');
        ...
        <View style={[ecs.sheet, { backgroundColor: c.surface }]}>
          ...
          <View style={ecs.headerRow}>
            <View style={[ecs.iconCircle, { backgroundColor: isAdd ? '#D1FAE5' : '#FEE2E2' }]}>
              <Ionicons name={isAdd ? 'wallet-outline' : 'trash-outline'} size={22} color={isAdd ? '#10B981' : '#EF4444'} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={[ecs.title, { color: c.text }]}>{isAdd ? 'Expense Detected' : 'Expense Removed?'}</Text>
              {total > 1 && <Text style={[ecs.subtitle, { color: c.textMuted }]}>{index} of {total}</Text>}
            </View>
          </View>
          {isAdd ? (
            <>
              {!!d?.snippet && (
                <View style={[ecs.snippetBox, { backgroundColor: c.surface2 ?? c.inputBg }]}>
                  <Text style={[ecs.snippetText, { color: c.textSub }]}>&ldquo;{d.snippet}&rdquo;</Text>
                </View>
              )}
              <Text style={[ecs.label, { color: c.textSub }]}>What was it for?</Text>
              <TextInput style={[ecs.input, { backgroundColor: c.inputBg, borderColor: c.border, color: c.text }]}
                value={expTitle} onChangeText={setExpTitle} placeholder="Expense title" placeholderTextColor={c.textMuted} />
              <Text style={[ecs.label, { color: c.textSub }]}>Amount (₹)</Text>
              <TextInput style={[ecs.input, { backgroundColor: c.inputBg, borderColor: c.border, color: c.text }]}
                value={expAmount} onChangeText={setExpAmount} keyboardType="decimal-pad" placeholder="0" placeholderTextColor={c.textMuted} />
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={ecs.pillScroll}>
                {EXPENSE_CATEGORIES.map((cat) => (
                  <TouchableOpacity key={cat} style={[ecs.pill, { borderColor: c.border }, expCategory === cat && ecs.pillActive]}
                    onPress={() => setExpCategory(cat)}>
                    <Text style={[ecs.pillText, { color: expCategory === cat ? '#fff' : c.textSub }]}>{cat}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
              {expCategory === 'Other' && (
                <TextInput
                  style={[ecs.input, { backgroundColor: c.inputBg, borderColor: c.border, color: c.text }]}
            ...
            <View style={[ecs.removeCard, { backgroundColor: c.surface2 ?? c.inputBg }]}>
              ...
              <Text style={[ecs.removeHint, { color: c.textMuted }]}>This expense is no longer mentioned in your note.</Text>
            </View>
          )}
          <View style={ecs.btnRow}>
            ...
            <TouchableOpacity style={[ecs.btnAdd, { backgroundColor: isAdd ? '#10B981' : '#EF4444' }]} onPress={handleConfirm}>
...
const ecs = StyleSheet.create({
  ...
  input: { borderWidth: 1.5, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontSize: 15, marginBottom: 14 },
  pillScroll: { marginBottom: 20 },
  pill: { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20, borderWidth: 1.5, marginRight: 8 },
  ...
  btnSkip: { flex: 1, borderWidth: 1.5, borderRadius: 14, paddingVertical: 14, alignItems: 'center' },
...
function ColorPicker({ visible, selected, onSelect, onClose }: {
  visible: boolean; selected: string;
      ...
      <View style={cp.box}>
        ...
        <View style={cp.swatches}>
          {NOTE_COLOR_OPTIONS.map((c) => (
            <TouchableOpacity key={c} style={[cp.swatch, { backgroundColor: c }, selected === c && cp.swatchActive]}
...
export default function NoteEditorScreen() {
  ...
  const [category, setCategory] = useState(existingNote?.category ?? initialCategory ?? noteDraft.category);
  ...
  const pendingExpensesRef = useRef<{ title: string; amount: number; category: ExpenseCategory }[]>([]);
  ...
  useEffect(() => { if (isNew) noteDraft.title    = title;    }, [title,    isNew]);
  useEffect(() => { if (isNew) noteDraft.color    = color;    }, [color,    isNew]);
  useEffect(() => { if (isNew) noteDraft.category = category; }, [category, isNew]);
  useEffect(() => { if (isNew) noteDraft.blocks   = blocks;   }, [blocks,   isNew]);
  ...
  const doSave = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      if (existingNote) {
        await updateNote(existingNote.id, { title: t, content: textContent, color, category });
        setHomeSegment('notes');
        router.replace('/(tabs)/home');
        return existingNote.id;
      } else {
        const note = await createNote({ title: t, content: textContent, color, category });
        clearNoteDraft();
        setHomeSegment('notes');
        router.replace('/(tabs)/home');
        return note.id;
      ...
      showAlert({ type: 'info', title: 'Something Went Wrong', message: 'Unable to save the note. Please try again.' });
      return null;
    } finally {
      setSaving(false);
    }
  }, [title, blocks, color, category, existingNote, createNote, updateNote, router, setHomeSegment, showAlert]);

  // ── Expense queue ─────────────────────────────────────────────
  const advanceQueue = useCallback((confirm: boolean, item: ExpenseQueueItem, expTitle: string, amount: number, cat: ExpenseCategory) => {
    if (item.type === 'add' && confirm) pendingExpensesRef.current.push({ title: expTitle, amount, category: cat });
      ...
      if (remaining.length === 0) {
        doSave().then((savedNoteId) => {
          for (const exp of pendingExpensesRef.current) {
            createExpense({ title: exp.title, amount: exp.amount, category: exp.category,
              date: new Date().toISOString().slice(0, 10), noteId: savedNoteId ?? null, source: 'note' });
          }
  ...
  }, [doSave, createExpense, deleteExpense]);

  const handleSave = useCallback(async () => {
    const t = title.trim();
    if (!t) { showAlert({ type: 'error', title: 'Title Required', message: 'Please add a title before saving.' }); return; }
    setDetecting(true);
    try {
      const allText = blocks.filter((b): b is TextBlock => b.type === 'text').map((b) => b.text).join('\n');
      ...
      const toAdd: ExpenseQueueItem[] = detected
        .filter((d) => !noteExpenses.some((e) => e.amount === d.amount && e.category === d.category))
        .map((d) => ({ type: 'add' as const, detected: d }));

      const queue = [...toAdd, ...toRemove];
      if (queue.length > 0) { totalDetectedRef.current = queue.length; setExpenseQueue(queue); return; }
    } finally {
      setDetecting(false);
    }
    await doSave();
  ...
  const handlePickImage = useCallback(async () => {
    ...
    if (status !== 'granted') {
      showAlert({ type: 'permission', title: 'Photo Access Required', message: 'Allow access to your photo library to attach images.' });
      return;
    }
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ['images'], quality: 0.8, allowsMultipleSelection: true, selectionLimit: 10,
    });
    if (!result.canceled && result.assets.length > 0) {
      ...
      if (newBlocks.length === 0) {
        showAlert({ type: 'error', title: 'Upload Failed', message: 'Could not upload image. Check your connection and try again.' });
        return;
      }
      if (failed > 0) {
        showAlert({ type: 'error', title: 'Some Images Failed', message: `${failed} image(s) could not be uploaded.` });
      }
      setBlocks((prev) => {
  ...
  return (
    <SafeAreaView style={[s.safe, { backgroundColor: color }]}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <View style={s.header}>
          ...
          <TouchableOpacity
            onPress={handleSave}
            style={[s.doneBtn, { backgroundColor: doneBg }, (!isDirty || saving || detecting) && { opacity: 0.4 }]}
        ...
        <View style={s.toolbarWrap}>
          <View style={[s.toolbar, { backgroundColor: toolbarBg }]}>
            <View style={s.toolbarLeft}>
              <TouchableOpacity style={s.toolIcon} activeOpacity={0.7} onPress={handlePickImage} disabled={uploadingImage}>
                {uploadingImage
                  ? <ActivityIndicator size="small" color={toolIconColor} />
                  : <Ionicons name="image-outline" size={22} color={toolIconColor} />}
              </TouchableOpacity>
              <TouchableOpacity onPress={() => setColorVisible(true)} style={[s.colorDot, { backgroundColor: color }]} activeOpacity={0.8} />
            </View>
            <View style={[s.toolDivider, { backgroundColor: isLight ? 'rgba(148,163,184,0.4)' : 'rgba(100,116,139,0.4)' }]} />
            <TouchableOpacity style={s.catBadge} onPress={() => setCatVisible(true)} activeOpacity={0.75}>
      ...
      <CategorySheet visible={catVisible} selected={category} onSelect={setCategory} onClose={() => setCatVisible(false)} />
      <ColorPicker    visible={colorVisible} selected={color}    onSelect={setColor}    onClose={() => setColorVisible(false)} />
      ...
      {expenseQueue.length > 0 && (
        <ExpenseConfirmSheet
          ...
          onAction={(confirm, t, amt, cat) => advanceQueue(confirm, expenseQueue[0], t, amt, cat)}
        />
      )}
      {AlertModal}
    </SafeAreaView>
...
const s = StyleSheet.create({
  ...
  colorDot: {
    ...
    borderWidth: 2.5, borderColor: 'rgba(0,0,0,0.18)',
  ...
  toolbar: {
    ...
    borderRadius: 32, paddingHorizontal: 16, paddingVertical: 12,
  ...
  toolIcon: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center', borderRadius: 20 },
